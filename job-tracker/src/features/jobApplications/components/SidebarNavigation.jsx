...
const NAV_ITEMS = [
  { key: 'dashboard', icon: LayoutDashboard, isActive: true },
...
const TooltipNavButton = ({ icon, label, isActive }) => {
  const IconComponent = icon;
  const base = 'group relative flex h-12 w-12 items-center justify-center rounded-full transition-all focus:outline-none focus-visible:ring-2 focus-visible:ring-blue-500 focus-visible:ring-offset-2 focus-visible:ring-offset-slate-900/10';
  ...
  return (
    <aside className="hidden min-h-screen w-24 shrink-0 items-center justify-center bg-transparent lg:flex">
      <div className="flex flex-col items-center gap-4 rounded-full bg-white/70 p-4 shadow-xl shadow-blue-500/5 ring-1 ring-slate-200/80 backdrop-blur">
        {NAV_ITEMS.map(({ key, icon, isActive }) => (
          <TooltipNavButton key={key} icon={icon} label={t(`navigation.items.${key}`)} isActive={isActive} />
        ))}
      </div>
    </aside>
  );
